...
@interface CompanyWebsiteViewController () <WKNavigationDelegate>
...
- (void)viewDidLoad {
    ...
    // 2. 加载数据
    NSString *companyURL = self.company.company_url;
    if (!IS_NULL_STRING(companyURL)) {
        NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:companyURL]];
...
- (WKWebView *)wkWebView {
    if (!_wkWebView) {
        ...
        // 2. 创建WKWebView
        CGFloat webViewH = BTSHEIGHT - NAVIGATION_BAR_HEIGHT - BTSTabBarHeightAdded;
