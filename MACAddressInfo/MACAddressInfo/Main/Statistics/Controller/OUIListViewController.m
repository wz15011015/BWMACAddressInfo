...
@interface OUIListViewController () {
    NSUInteger _pageNum;
...
@implementation OUIListViewController
...
- (void)loadUI {
    self.navigationItem.title = BTSLocalizedString(@"OUI List", nil);
...
- (void)loadData {
    ...
    // 加载第一页数据
    NSArray *dataArr = [DatabaseManagerInstance queryOUIWithPageNum:_pageNum pageSize:_pageSize];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.infoType = OUIInfoOUICode;
    
    OUIModel *model = self.dataArr[indexPath.row];
    cell.ouiModel = model;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    OUIModel *oui = self.dataArr[indexPath.row];
    OUIInfoViewController *vc = [[OUIInfoViewController alloc] init];
    vc.oui = oui;
